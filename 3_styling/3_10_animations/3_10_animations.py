...
class AnimatedButton(ctk.CTkButton):
    def __init__(self, parent, light_path, dark_path):
        ...
        super().__init__(
            ...
            command=self.infinite_animation,
        )
    ...
    def import_folders(self, light_path, dark_path):
        ...
        for path in (light_path, dark_path):
            for _, __, image_data in walk(path):
                sorted_data = sorted(
                    image_data, key=lambda item: int(item.split(".")[0][-5:])
                )
    ...
    def infinite_animation(self):
        self.frame_index += 1
        self.frame_index = (
            0 if self.frame_index >= self.animation_length else self.frame_index
        )
        self.configure(image=self.frames[self.frame_index])
        self.after(20, self.infinite_animation)
...
AnimatedButton(
    window,
    "/home/howard/sandbox/python_gui/3_styling/3_10_animations/light",
    "/home/howard/sandbox/python_gui/3_styling/3_10_animations/dark",
)
